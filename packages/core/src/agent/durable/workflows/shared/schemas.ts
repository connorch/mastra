...
export const baseIterationStateSchema = z.object({
  ...
  // PATCH(walton): JSON-safe request context snapshot, carried across iterations so
  // cross-isolate durable steps can restore it. Without this the field is stripped
  // here and `restoreRequestContext(input.requestContextEntries)` sees undefined on
