  ...
  getFunction(): ReturnType<Inngest['createFunction']> {
    ...
    this.function = this.inngest.createFunction(
      ...
      async ({ event, step, attempt }) => {
        ...
        // PATCH(walton): resume events ship no stepResults/initialState (see
        // _resumeAndSendEvent / executeWorkflowStep) — rehydrate them from this
        // workflow's own persisted snapshot so multi-MB suspended state never
        // rides through Inngest payload limits. Deliberately NOT inside a
